...
import { restoreReportedPostController } from '../../src/features/posts/controllers/reportedPosts.controller';
...
describe('POST /admin/reported/restore', () => {
    const mockedAuthenticateJWT = jest.mocked(authenticateJWT);
    const mockedRestoreReportedPostController = jest.mocked(restoreReportedPostController);

    beforeEach(() => {
        jest.clearAllMocks();
    });

    it('should return 200 when post is successfully restored', async () => {
        ...
        mockedRestoreReportedPostController.mockImplementation(
